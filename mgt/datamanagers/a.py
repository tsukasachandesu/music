...
def cal_key(piano_roll: PianoRoll,
    ...
    for name in key_names:
        ...
        mode = name.split()[1]
        # print(f"Cal Key: {key} - {mode}")
        # ----------- key_index, pitch spelling ----------- # 
        if mode == 'minor':
        ...
        # print(f"Cal Key,  key_index = : {key_index}")
    ...
    # print("Method3-MiDI Miner:", key_name, key_pos, key_shift_for_ce)
...
def cal_tension(file_name: str,
    ...
    try:
        ...
        if True:
            ...
            key_change_bar = detect_key_change(key_diff, diameters, start_ratio=0.5)
            if key_change_bar != -1:
                ...
                changed_key_name, changed_key_pos, changed_note_shift = get_key_index_change(pm, change_time,
                                                                                             sixteenth_time)
                if changed_key_name != key_name:
                    m = int(change_time // 60)
                    s = int(change_time % 60)
                    print(f'key changed, change time is {m} minutes, {s} second')
        ...
        pickle.dump(window_time[:len(total_tension)], open(os.path.join(new_output_folder, name_split[0] + '.time'), 'wb'))
        '''
        ...
        return [total_tension, diameters, centroid_diff, key_name, change_time, key_change_bar, changed_key_name,
                new_output_folder]
...
def note_to_key_pos(note_indices: List[int], key_pos: int) -> ndarray:
    ...
    diffs = np.linalg.norm(np.array(note_positions) - key_pos, axis=1)  # 计算欧氏距离，axis =1 表示按行向量来进行处理，求多个行向量的范数
    return diffs
...
def tonality_pitch_shift_cal(keymode):
    pass

# ----------------- [End] MIDI Miner toolkit --------------------# 


def tonality_cal_lead_job(midi_path):
    file_name = midi_path
    ...
    # Cal Tension
    # ----------------------------------------------------------------------------------------------------------
    # key_name = all_key_names  # all major or minor
    result = cal_tension(
...
if __name__ == "__main__":
    ...
    # The default tension calculation window size is one bar. The tension calculation length window could be set by parameter -w.
    # -w 1 set the window to 1 beat, -w 2 means 2 beat, and -1 is default for a downbeat (1 bar).
    ...
    for file_name in all_names:
        ...
        try:
            if args.key_name == '':
                ...
                result = cal_tension(
                    file_name, piano_roll, sixteenth_time, beat_time, beat_indices, down_beat_time, down_beat_indices,
                    args.output_folder, args.window_size, key_name)

            else:
                result = cal_tension(
                    file_name, piano_roll, sixteenth_time, beat_time, beat_indices, down_beat_time, down_beat_indices,
                    args.output_folder, args.window_size, [args.key_name])
            ...
            if args.key_name == '':
                ...
                result = cal_tension(
                    file_name, piano_roll, sixteenth_time, beat_time, beat_indices, down_beat_time, down_beat_indices,
                    args.output_folder, args.window_size, [result_key])
