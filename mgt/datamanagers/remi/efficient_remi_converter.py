...
class EfficientRemiConverter(object):
    ...
    def convert_to_remi_items(self, events):
        ...
        for index, event in enumerate(events):
            ...
            elif event.name == 'Position' and len(events) > index + 2 and events[index + 1].name == 'Chord':
                original_events = [event, events[index + 1]]
        ...
        for index, word in enumerate(efficient_remi_words):
            ...
            if self.config.remove_velocity:
                if word.startswith('Note On'):
                    ...
                    result.append("Note Velocity_32")  # Default velocity of 30
