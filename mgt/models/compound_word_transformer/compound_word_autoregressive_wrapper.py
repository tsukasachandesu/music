...
class CompoundWordAutoregressiveWrapper(nn.Module):
    ...
    def generate(self, prompt, output_length=100, selection_temperatures=None, selection_probability_tresholds=None):
        ...
        for _ in range(output_length):
            ...
            print(h.shape)
            e = gumbel_sample(top_k(e[:, -1:, :], thres = filter_thres), temperature = temperature)
            print(h.shape)
            f = gumbel_sample(top_k(f[:, -1:, :], thres = filter_thres), temperature = temperature)
            print(h.shape)
            g = gumbel_sample(top_k(g[:, -1:, :], thres = filter_thres), temperature = temperature)
            print(h.shape)
            h = gumbel_sample(top_k(h[:, -1:, :], thres = filter_thres), temperature = temperature) 
            print(h.shape)
