...
class CompoundWordAutoregressiveWrapper(nn.Module):
    ...
    def train_step(self, x, **kwargs):
        ...
        print(f.shape)
        ff = torch.nn.functional.one_hot(x[:, 1:, 1], num_classes=6914)[:,:,1:] + torch.nn.functional.one_hot(x[:, 1:, 2], num_classes=6914)[:,:,1:] + torch.nn.functional.one_hot(x[:, 1:, 3], num_classes=6914)[:,:,1:] + torch.nn.functional.one_hot(x[:, 1:, 4], num_classes=6914)[:,:,1:] + torch.nn.functional.one_hot(x[:, 1:, 5], num_classes=6914)[:,:,1:] + torch.nn.functional.one_hot(x[:, 1:, 6], num_classes=6914)[:,:,1:]
        print(ff.shape)
        print(ff[:,:,1:-1].shape)
        loss1 = calculate_loss1(f, ff[:,:,1:-1].float(), type_mask(target)) *0.5
